...
public class RoutesController : ControllerBase
{
    ...
    public ActionResult<string> CreateComment([FromBody] HTTPCommentData comment) {
        ...
        _dbcontext.CreateComment(comment.Content, comment.CommentId, Int32.Parse(HttpContext.User.Claims.First(c => c.Type == ClaimTypes.NameIdentifier).Value), DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
    ...
    public ActionResult<string> CreateArticle([FromBody] HTTPArticleData article) {
        ...
        _dbcontext.CreateArticle(article.Title, article.Content, Int32.Parse(HttpContext.User.Claims.First(c => c.Type == ClaimTypes.NameIdentifier).Value), DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
